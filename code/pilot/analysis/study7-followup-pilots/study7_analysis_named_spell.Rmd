...
```{r, warnings = FALSE}
data_path_cf <- paste0(base_path, "/data/data-study7-follow-up-pilots/magical_named_spell/JO-mbc-main-study7-magical-spel_50N")
...
dat <- d %>% 
  ...
  filter(condition %in% c("named_spell")) %>%
...
main_fig <- ggsave("figures/named_spell.png", width = 6, height = 5, plot = main_fig, bg = "white")
...
p_named_spell <- dat_summarized_by_item_levels %>% 
  ggplot(aes(x =condition, y = mean_proportion, fill = response)) +
  ...
  labs(y = "proportion",
       title = "Magical intervention - named spell wording") +
  facet_wrap(~facet_label, nrow = 3)

p_named_spell

ggsave(
  filename = "figures/p_named_spell_item-level_stacked.pdf",
  plot = p_named_spell,          
  device = "pdf",           
  width = 20,               
  height = 15,             
...
ggsave(
  filename = "figures/p_named_spell_item-level_stacked.png",
  plot = p_named_spell,  
  bg = "white",
  width = 20,               
  height = 15,             
