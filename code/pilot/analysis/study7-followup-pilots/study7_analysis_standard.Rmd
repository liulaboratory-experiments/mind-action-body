...
```{r, warnings = FALSE}
data_path_cf <- paste0(base_path, "/data/data-study7-follow-up-pilots/magical_standard/JO-mbc-main-study7-magical-stan_50N")
...
dat <- d %>% 
  ...
  filter(condition %in% c("standard")) %>%
...
main_fig <- ggsave("figures/standard.png", width = 6, height = 5, plot = main_fig, bg = "white")
...
p_standard <- dat_summarized_by_item_levels %>% 
  ggplot(aes(x =condition, y = mean_proportion, fill = response)) +
  ...
  labs(y = "proportion",
       title = "Magical intervention - standard wording") +
  facet_wrap(~facet_label, nrow = 3)

p_standard

ggsave(
  filename = "figures/p_standard_item-level_stacked.pdf",
  plot = p_standard,          
  device = "pdf",           
  width = 20,               
  height = 15,             
...
ggsave(
  filename = "figures/p_standard_item-level_stacked.png",
  plot = p_standard,  
  bg = "white",
  width = 20,               
  height = 15,             
