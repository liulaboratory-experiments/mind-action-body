...
```{r, warnings = FALSE}
data_path_cf <- paste0(base_path, "/data/data-study7-follow-up-pilots/magical_pilot/JO-mbc-main-study7-magical-pilot_50N")
...
main_fig <- dat_summarized %>%
  ...
  scale_x_discrete(labels = "Magical") +
...
main_fig <- ggsave("figures/pilot-thin.png", width = 3, height = 5, plot = main_fig, bg = "white")
...
p_pilot <- dat_summarized_by_item_levels %>% 
  ggplot(aes(x =condition, y = mean_proportion, fill = response)) +
  ...
  labs(y = "proportion",
       title = "Magical intervention - pilot wording") +
  facet_wrap(~facet_label, nrow = 3)

p_pilot

ggsave(
  filename = "figures/p_pilot_item-level_stacked.pdf",
  plot = p_pilot,          
  device = "pdf",           
  width = 20,               
  height = 15,             
...
ggsave(
  filename = "figures/p_pilot_item-level_stacked.png",
  plot = p_pilot,  
  bg = "white",
  width = 20,               
  height = 15,             
...
model_effect_of_magical_intervention <- glmer(response ~ 1 + (1 | subject_id), 
      family = binomial(link = 'logit'), 
      data = d_model)

summary(model_effect_of_magical_intervention)
plogis(fixef(model_effect_of_magical_intervention)[1])
...
```{r}
effects_data <- allEffects(model_effect_of_magical_intervention)
...
model_sim <- extend(model_effect_of_magical_intervention, along = "subject_id", n = 150)
length(unique(simr::getData(model_sim)$subject_id)) #confirm that N is now 150

curve_magical <- powerCurve(model_sim, fixed("(Intercept)",
                                               "z"),
