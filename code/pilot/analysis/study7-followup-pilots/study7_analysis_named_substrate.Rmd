...
```{r, warnings = FALSE}
data_path_cf <- paste0(base_path, "/data/data-study7-follow-up-pilots/magical_named_substrate/JO-mbc-main-study7-magical-sub_50N")
...
dat <- d %>% 
  ...
  filter(condition %in% c("named_substrate")) %>%
...
main_fig <- dat_summarized %>%
  ggplot(aes(x = condition, y = mean_proportion, fill = response)) +
    geom_bar(stat = "identity", colour = "black") +
    scale_fill_manual(values = c("grey80", "#000000")) + #or whatever you'd like!
  scale_x_discrete(labels = c("Giving Creature Ability" = "magical", 
                              "Making Someone do Something" = "ordinary")) +
    geom_errorbar(data = filter(dat_summarized, response == "causal"), aes(ymax = mean_proportion + se,
                                                                           ymin = mean_proportion - se),
                  stat = "identity", position = "identity", width = 0.3) +
...
main_fig <- ggsave("figures/named_substrate.png", width = 6, height = 5, plot = main_fig, bg = "white")
...
p_named_substrate <- dat_summarized_by_item_levels %>% 
  ggplot(aes(x =condition, y = mean_proportion, fill = response)) +
  ...
  labs(y = "proportion",
       title = "Magical intervention - named substrate wording") +
  facet_wrap(~facet_label, nrow = 3)

p_named_substrate

ggsave(
  filename = "figures/p_named_substrate_item-level_stacked.pdf",
  plot = p_named_substrate,          
  device = "pdf",           
  width = 20,               
  height = 15,             
  units = "in")
...
ggsave(
  filename = "figures/p_named_substrate_item-level_stacked.png",
  plot = p_named_substrate,  
  bg = "white",
  width = 20,               
  height = 15,             
