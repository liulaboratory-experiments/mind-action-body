...
spp_26 <- dat_summarized %>%
  ...
  labs(y = "Proportion",
       x = "Condition",
       fill = "Response") +
  scale_x_discrete(labels = c("Inference" = "Inf.", "Prediction" = "Pred.")) +
  ylim(0, 1) +
  theme(
...
```{r}
#MODEL
d_following <- read_csv(here("code", "pilot", "analysis", 
                             "study7-followup-pilots", "following.csv")) %>% 
...
d_joined <- rbind(d_following, d_prediction)
