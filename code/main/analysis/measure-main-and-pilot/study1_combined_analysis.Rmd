...
What we do: calculate euclidean distances within subjects, then immediately normalize, then average to get mean rdm. Then we min-max normalize that mean before plotting. (so 2 normalizations are done)
...
compute_ci(df_causal %>% 
             filter(domain_itemA == "bio", domain_itemB == "mind", 
                    itemB %in% c("choose what to do", "think about something", 
                                 "remember something")),
                    # itemA %in% c("take a walk")),
           "body → cognition")
...
## Stats - for causal signatures paper
...
d_sub <- d %>%
  ...
  mutate(
    itemB = factor(itemB, levels = c("experience pain", "become hungry")),
