...
# Preceding Causal Items and Similar Items - for Explanation, Intervention, Counterfactuals Tasks
...
## troubleshooting plot -- for choose what to do 
```{r}
long_list_items <- df_item_pair_means_freesort_minus_causal %>% 
  group_by(itemB) %>% 
  arrange(desc(freesort_minus_causal)) %>% 
  ...
  mutate(cd = round(cd, 2),
         ...
         fd_minus_cd = round(fd_minus_cd, 2)) %>% 
  relocate(fd, .before = cd) %>% 
  filter(target == "choose what to do") 
...
long_list_items <- long_list_items %>% 
  gt() %>% 
  data_color(
    columns = target,  #apply color to this column
    colors = color_map
  )

long_list_items 

gtsave(long_list_items, "figures/item-selection/preceding_long-list-normalized-choose.png")
# gtsave(long_list_items, "figures/item-selection/preceding_long-list-non-normalized.png")
...
# Following Causal Items and Similar Items - for Prediction Task
