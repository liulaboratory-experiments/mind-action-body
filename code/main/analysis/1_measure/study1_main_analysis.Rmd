...
# paths
## script inputs
base_path <- here("code", "main", "analysis", "1_measure")
...
## Figure: Mean freesort rdm with numbers
...
## Figure: Mean causal rdm with numbers

```{r}

png(
  file = paste0(rdms_outputs_path, "/causal/averageCausalRDM-numbers.png"),
  width = 12,
  height = 12,
  units = "in",
  res = 300,
...
df_mind_bio <- df_causal %>%
  filter(
    (domain_itemA == "mind" & domain_itemB == "bio") |
    (domain_itemA == "bio" & domain_itemB == "mind")
  ) %>%
  mutate(direction = case_when(
    domain_itemA == "mind" & domain_itemB == "bio" ~ "mind_to_bio",
    domain_itemA == "bio" & domain_itemB == "mind" ~ "bio_to_mind"
  ),
  direction = factor(direction,
                     levels = c("bio_to_mind", "mind_to_bio")))

model_mind_bio <- lmerTest::lmer(
  causal_distance ~ direction + (1 | subject_id),
  data = df_mind_bio
)

summary(model_mind_bio)
parameters::standardize_parameters(model_mind_bio)
...
write.csv(freesort_task_order_correlations, "freesort_task_order_correlations.csv")
