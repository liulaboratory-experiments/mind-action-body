...
1. Combines raw data from the pilot and main studies `study1_pilot_data_raw` and `study1_main_data_raw.csv`. The codebooks for the raw datasets are available at `data/study1_data_raw_codebook` and XXX (generate codebook for the other one!)  
2. Parses it into a combined (i.e. freesort + causation) data frame and saves into csv `combined_data_processed_step1.csv`. The codebook for this data frame is available at `/snapshots_study1_preprocessing_analysis/data_processed_step1_codebook`.  
...
- NB: Total N should be reported as 152, not 155 (as is in preregistrations)! (since we lost N = 2 in main round)
...
data_path_1 <- paste0(base_path, "/data/study1-measure/study1_main_data_raw.csv")
data_path_2 <- paste0(base_path, "/data/study1-measure/study1_pilot_data_raw.csv")
...
#paths: data outputs
analysis_path <- paste0(base_path, "/analysis/study1-measure-main-and-pilot")
rdm_figures_outputs_path <- paste0(analysis_path, "/figures/theoretical-rdms/")
combined_output_csv <- "combined_data_processed_step1"
snapshots_path <- paste0(analysis_path, "/snapshots_study1_preprocessing_analysis")
...
plot_rdm_theory(d_all_theories, "theor4_cosine_similarity", desired_order, "Cosine Distance")
plot_rdm_theory(d_all_theories, "theor1_Mind-Body-Action", desired_order, "3 categories: MBA")
plot_rdm_theory(d_all_theories, "theor2_PerCog-OdActSpAct-BodyStimBodynoStim", desired_order, "6 categories: PCOS")
plot_rdm_theory(d_all_theories, "theor3_Physical-Ethereal", desired_order, "2 categories: PE")
# save them
#output_folder <- paste0(here(rdm_outputs_path), "/figures/theoretical-rdms/") -> delete this

theories <- list(
  list(column_name = "theor4_cosine_similarity", order = desired_order, title = "Cosine Distance"),
  list(column_name = "theor1_Mind-Body-Action", order = desired_order, title = "3 categories: MBA"),
  list(column_name = "theor2_PerCog-OdActSpAct-BodyStimBodynoStim", order = desired_order, title = "6 categories: PCOS"),
  list(column_name = "theor3_Physical-Ethereal", order = desired_order, title = "2 categories: PE")
)

for (i in seq_along(theories)) {
  theory <- theories[[i]]
  output_file <- paste0(rdm_figures_outputs_path, "theory_", i, ".png")
