...
1. Takes raw data from the main study `study1_main_data_raw.csv`. The codebook for the raw data is available at `data/study1_data_raw_codebook`.  
2. Parses it into a combined (i.e. freesort + causation) data frame and saves into csv `main_data_processed_step1.csv`. The codebook for this data frame is available at `/snapshots_study1_preprocessing_analysis/data_processed_step1_codebook`.  
...
Study description:    
- This study was ran on 6/30/25 on Connect CloudResearch, N = 102 participants (105 were recruited but 2 lost due to technical issues causing data loss)
...
data_path <- paste0(base_path, "/data/1_measure/main_data_raw.csv")
...
#paths: data outputs
analysis_path <- paste0(base_path, "/analysis/1_measure-main-only")
rdm_figures_outputs_path <- paste0(analysis_path, "/figures/theoretical-rdms/")
combined_output_csv <- "main_data_processed_step1"
