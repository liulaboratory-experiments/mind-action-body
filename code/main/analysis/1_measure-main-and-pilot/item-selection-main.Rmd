...
What this script: groups by item pairs (this is also what we do in other scripts to get the mean rdm) then normalizes. This is a second normalizing in addition to the subject-wise normalizing done in the original script. In summary, this script gets the mean rdm then normalizes that, then generates tables. 
...
# Preceding - for Experiment 2 (Explanation), 3 (Counterfactual), and 5 (Intervention)
...
# gtsave(long_list_items, "figures/item-selection/preceding_long-list-normalized.png")
# gtsave(long_list_items, "figures/item-selection/preceding_long-list-non-normalized.png")
...
df_table <- short_list_table %>%
  ...
  cols_label(
    ...
    causal_choice = md("CC item<br>(CC distance, Freesort Distance)"),
    similar_choice = md("CO item<br>(CC distance, Freesort Distance)")
  ) %>%
...
gtsave(df_table, "figures/item-selection/preceding_short-list-non-normalized.png")
...
#save as csv
write_csv(file = "snapshots_study1_main_analysis/preceding_short_list_domain_constraints-non-normalized.csv",
...
gtsave(df_table_domain_constraints,
       "figures/item-selection/preceding_short-list-constrained-non-normalized.png")
```

# Following - for Experiment 5 (Prediction)
...
gtsave(long_list_items, "figures/item-selection/following_long-list-Prediction-task-non-normalized.png")
...
gtsave(df_table, "figures/item-selection/following_short-list-Prediction-task-non-normalized.png")
...
write_csv(file = "snapshots_study1_main_analysis/following_short_list_Prediction_domain_constraints-non-normalized.csv",
          short_list_selected_items_domain_constraints)
...
gtsave(df_table_domain_constraints,
       "figures/item-selection/following_short-list-Prediction-constrained-non-normalized.png")
```
...
gtsave(df_table, "figures/item-selection/preceding_short-list-normalized.png")
...
#save as csv
write_csv(file = "snapshots_study1_main_analysis/preceding_short_list_domain_constraints-normalized.csv",
          short_list_selected_items_domain_constraints)
...
gtsave(df_table_domain_constraints,
       "figures/item-selection/preceding_short-list-constrained-normalized.png")
```
...
gtsave(long_list_items, "figures/item-selection/following_long-list-Prediction-task-normalized.png")
...
gtsave(df_table, "figures/item-selection/following_short-list-Prediction-task-normalized.png")
...
write_csv(file = "snapshots_study1_main_analysis/following_short_list_Prediction_task_domain_constraints-normalized.csv",
          short_list_selected_items_domain_constraints)
...
gtsave(df_table_domain_constraints,
       "figures/item-selection/following_short-list-Prediction-task-constrained-normalized.png")
```
